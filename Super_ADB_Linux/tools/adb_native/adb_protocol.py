...
def _定位密钥路径():
    ...
    if getattr(sys, 'frozen', False):
        ...
        if not os.path.isfile(new_path):
            ...
            _up3 = os.path.dirname(os.path.dirname(os.path.dirname(base)))
            candidates = [
                # 旧打包版路径：_internal/config/（__file__ 推导）
                os.path.join(_meipass, 'config', fname),
                # 开发机：exe 在源码树内（平台根/build_tools/dist/Super_ADB）→ 上溯 3 级到平台根 config/
                os.path.join(_up3, 'config', fname),
                # 向后兼容：目录改英文名之前的旧中文路径（保留中文字面量）
                os.path.join(base, '配置', fname),
                os.path.join(_meipass, '配置', fname),
                os.path.join(_up3, '配置', fname),
            ]
    ...
    if not os.path.isfile(_new):
        # 向后兼容：目录改英文名之前的旧中文路径，命中就继续用，避免设备需重新授权
        _legacy = os.path.join(_项目根, '配置', fname)
