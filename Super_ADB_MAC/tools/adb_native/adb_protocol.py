...
def _定位密钥路径():
    ...
    if getattr(sys, 'frozen', False):
        ...
        if not os.path.isfile(new_path):
            ...
            candidates = [
                ...
                os.path.join(getattr(sys, '_MEIPASS', ''), 'config', fname),
                # 开发机：exe 在源码树内（平台根/build_tools/dist/Super_ADB）→ 上溯 3 级到平台根 config/
                os.path.join(_src_root, 'config', fname),
                # 向后兼容：目录名英文化之前的旧中文配置目录 配置/
                os.path.join(getattr(sys, '_MEIPASS', ''), '配置', fname),
                os.path.join(_src_root, '配置', fname),
            ]
    ...
    return os.path.join(_项目根, 'config', fname)
