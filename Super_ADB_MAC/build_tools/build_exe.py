...
def install(main):
    ...
    # 相对名 外部扩展 会落到 _internal/vendor，与源码目录结构一致
    ...
    if sys.platform == 'darwin':
        ...
        cmd = f'pyinstaller -w -i "{icon}" -n {name} --distpath "{base_dir}/build_tools/dist" --workpath "{base_dir}/build_tools/build" {hidden} {hooks} {runtime_hooks} {excludes} {res_arg} {data_arg} {path_args} "{main}"'
    else:
        # Windows: 生成 .exe
