...
def install(main):
    ...
    # 各子目录（dialogs/pages/monitoring/tools/ui）均含 __init__.py 成为正规包，
    # PyInstaller 通过根包路径自动发现所有子包模块。
    here = os.path.dirname(os.path.abspath(__file__))
    base_dir = os.path.dirname(here)
    # ui 目录也加入 paths，因为 Super_ADB.py 中是裸导入 import png_rc
    ...
    # ★ PCAP 解析已弃用 scapy，改用纯 Python 的 tools.lightweight_pcap_parser（零依赖，
    ...
    # 官方 scrcpy 投屏二进制（vendor/scrcpy/，~25MB）：投屏功能直接调用，随包分发。
    # tcpdump 抓包二进制（vendor/tcpdump/，~4MB）：无 root 设备自动推送，随包分发。
    # PyInstaller 的 SRC:DST 分隔符在 Windows 上为 ';'、其余平台为 ':'。
    # 注意：adb_tools.py（原 adb_utils.py，位于 tools/）用 __file__ 定位 vendor/，
    # 打包后 __file__ 在 _internal/ 顶层，所以 vendor 必须放到
    # Super_ADB_Win/vendor 才能和源码目录结构保持一致。
    ...
    # 注意：目标路径不能带前导 / —— Windows 下会静默失败导致 vendor 没进包，
    # 相对名 vendor 会落到 _internal/vendor，与源码目录结构一致
    ...
    _待清目录 = [os.path.join(base_dir, 'build_tools', 'dist', name)]
    _平台名 = f'Super_ADB_{_平台后缀()}'
    _待清目录.append(os.path.join(base_dir, 'build_tools', 'dist',
