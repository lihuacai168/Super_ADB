...
# 打包信息文件候选（按新→旧顺序）。中文字面量为向后兼容常量：
# v1 用中文目录名 配置/ 与中文文件名 打包信息.json，老用户升级后仍能读到版本号，勿删。
# macOS 上打包信息写在 <.app>/Contents/Resources/config/，而 _base 是
# Contents/MacOS，故第一条用 ../Resources 相对过去（Windows/Linux 无此目录，
# 自然落空进入下一条）。改位置的原因见 build_tools/build_exe.py：
# Contents/MacOS 下的非可执行文件会让 codesign --verify --deep --strict 失败。
...
def _打包信息路径():
    """返回第一个存在的打包信息文件路径，都不存在返回 None。"""
    if getattr(sys, 'frozen', False):
        # 打包模式：配置文件在可执行文件旁边
        _base = os.path.dirname(sys.executable)
    else:
        # 源码模式：本文件位于 dialogs/ 下，配置在项目根
        _base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    for _dir, _name in _BUILD_INFO_CANDIDATES:
        _p = os.path.join(_base, _dir, _name)
...
class 关于对话框(QDialog, 无边框缩放Mixin):
    ...
    def _load_qr_pixmap(self):
        ...
        资源 alias = qrcode.jpg，源文件 ui/wechat_qrcode.jpg，由 ui/png.qrc 编译进 png_rc.py。
